...
function calculateAppealValue() {
  ...
  for (let i = 1; i <= 6; i++) {
    ...
    if (cardType && cardData[cardType] && cardData[cardType].stats) {
      ...
      if (
        centerCard &&
        centerCard.centerCharacteristic &&
        centerCard.centerCharacteristic.effects
      ) {
        for (const effect of centerCard.centerCharacteristic.effects) {
          if (effect.type === 'appealBoost') {
            ...
            if (effect.target === 'all') {
              ...
              effect.target === '102期' &&
              ['乙宗梢', '藤島慈', '夕霧綴理'].includes(character)
            ) {
              shouldApplyBoost = true
              boostReason = '102期生'
            } else if (
              effect.target === '103期' &&
              ['日野下花帆', '村野さやか', '大沢瑠璃乃'].includes(character)
            ) {
              shouldApplyBoost = true
              boostReason = '103期生'
            } else if (
              effect.target === '104期' &&
              ['百生吟子', '徒町小鈴', '安養寺姫芽'].includes(character)
            ) {
              shouldApplyBoost = true
              boostReason = '104期生'
            } else if (
              effect.target === 'スリーズブーケ' &&
              ['乙宗梢', '日野下花帆', '百生吟子'].includes(character)
            ) {
              shouldApplyBoost = true
              boostReason = 'スリーズブーケ'
            } else if (
              effect.target === 'DOLLCHESTRA' &&
              ['夕霧綴理', '村野さやか', '徒町小鈴'].includes(character)
            ) {
              shouldApplyBoost = true
              boostReason = 'DOLLCHESTRA'
            } else if (
              effect.target === 'みらくらぱーく！' &&
              ['藤島慈', '大沢瑠璃乃', '安養寺姫芽'].includes(character)
            ) {
              shouldApplyBoost = true
              boostReason = 'みらくらぱーく！'
            } else if (
              effect.target === 'Edel Note' &&
              ['セラス 柳田 リリエンフェルト', '桂城泉'].includes(character)
            ) {
              shouldApplyBoost = true
...
function updateBaseAP() {
  ...
  if (musicSelect === 'custom') {
    ...
    const comboInput = document.getElementById(
      `customCombo${difficulty.charAt(0).toUpperCase() + difficulty.slice(1)}`,
    )
    ...
    musicData[musicSelect] &&
    musicData[musicSelect].combos &&
    musicData[musicSelect].combos[difficulty]
  ) {
...
class Game {
    ...
    if (this.verbose) {
      ...
      if (card.apCost) {
        ...
        if (this.centerCharacter) {
          const centerCard = this.cards.find(
            (c) => c.config && c.config.character === this.centerCharacter,
          )

          if (centerCard && centerCard.config.centerCharacteristic) {
            const apReduceEffect = centerCard.config.centerCharacteristic.effects.find(
              (e) => e.type === 'apReduce',
            )
      ...
      this.currentTurnLog.push(
        `<div class="log-turn-header"><span class="turn-number">${this.turn + 1}</span> ${phase} <span class="log-card-name">${cardName}</span>${apDisplayHtml}</div>`,
      )
    }
  ...
  doVoltageBoost(value, times = 1) {
    ...
    if (this.verbose) {
      ...
      this.currentTurnLog.push(
        `<div class="log-action log-boost-action"><div class="log-voltage-boost">ボルテージブースト</div><div class="boost-values">+${percent}% → ${total}%</div></div>`,
      )
    }
  }
  ...
  getScore(value) {
    ...
    if (this.verbose) {
      ...
      const totalScoreBoostPercent = (
        (1 + this.scoreBoost[this.scoreBoostCount - 1]) *
        100
      ).toFixed(2)
      const totalVoltageLevelPercent = ((1 + voltageLevel / 10) * 100).toFixed(2)
      this.currentTurnLog.push(
        `<div class="log-action log-boost-action"><div class="log-score-gain">スコア獲得</div><div class="score-gain-values">+${score.toLocaleString()} → ${this.score.toLocaleString()}</div></div>`,
      )
  ...
  getVoltage(value) {
    ...
    if (this.verbose) {
      ...
      this.voltageBoostCount -= 1
      this.currentTurnLog.push(
        `<div class="log-action"><span class="log-voltage-gain">ボルテージ獲得: +${voltagePt} → ${this.voltagePt}</span></div>`,
      )
  ...
  getAP(value) {
    ...
    if (this.verbose) {
      ...
      this.currentTurnLog.push(
        `<div class="log-action log-boost-action"><div class="log-ap-gain">💎 AP獲得</div><div class="ap-gain-values">+${formatAP(actualValue)} → ${formatAP(this.apAcquired)}</div></div>`,
      )
    }
  }
  ...
  activateCenterSkills(timing) {
    ...
    for (const card of this.cards) {
      if (
        card.config &&
        card.config.character === this.centerCharacter &&
        card.config.centerSkill &&
        card.config.centerSkill.when === timing
      ) {
        ...
        if (this.verbose) {
          ...
          this.currentTurnLog.push(
            `<div class="log-turn-header"><span class="turn-number center-skill">センタースキル</span>${feverIcon} <span class="log-card-name">${card.displayName}</span></div>`,
          )
        }
        ...
        for (let i = 0; i < centerSkill.effects.length; i++) {
          this.processCenterSkillEffect(
            centerSkill.effects[i],
            card.centerSkillLevel,
            card.centerSkillValues,
            i,
          )
        }
  ...
  processCenterSkillEffect(effect, skillLevel, centerSkillValues, effectIndex = 0, prefix = '') {
    ...
    switch (effect.type) {
      case 'scoreGain':
        const scoreValue =
          centerSkillValues[key] !== undefined ? parseFloat(centerSkillValues[key]) : effect.value
      ...
      case 'voltageGain':
        const voltageValue =
          centerSkillValues[key] !== undefined ? parseInt(centerSkillValues[key]) : effect.value
      ...
      case 'voltagePenalty':
        ...
        if (this.verbose) {
          this.currentTurnLog.push(
            `<div class="log-action"><span style="color: #e74c3c;">ボルテージ-${effect.value} (残り: ${this.voltagePt})</span></div>`,
          )
        }
        break
      ...
      case 'mentalReduction':
        ...
        if (this.verbose) {
          this.currentTurnLog.push(
            `<div class="log-action"><span style="color: #e74c3c;">メンタル${effect.value}%減少 → ${this.mental}%</span></div>`,
          )
        }
        break

      case 'appealBoost':
        const appealBoostValue =
          centerSkillValues[key] !== undefined ? parseFloat(centerSkillValues[key]) : effect.value
        this.appeal = Math.floor(this.appeal * (1 + appealBoostValue))
        if (this.verbose) {
          this.currentTurnLog.push(
            `<div class="log-action"><span style="color: #2196F3;">アピール値${Math.round(appealBoostValue * 100)}%上昇 → ${this.appeal}</span></div>`,
          )
        }
        break
  ...
  evaluateCenterSkillExpression(expr) {
    ...
    switch (trimmed) {
      case 'mental':
        return this.mental
      case 'voltageLevel':
        return this.getVoltageLevel()
      case 'voltagePt':
        return this.voltagePt
      case 'turn':
        return this.turn
      default:
...
class GenericCard extends Card {
  ...
  do(game) {
    ...
    for (let i = 0; i < effects.length; i++) {
      ...
      switch (effect.type) {
        ...
        case 'scoreBoost':
          const scoreBoostValue =
            this.skillValues[`effect_${i}_value`] !== undefined
              ? parseFloat(this.skillValues[`effect_${i}_value`])
              : effect.value
        ...
        case 'voltageGain':
          const voltageGainValue =
            this.skillValues[`effect_${i}_value`] !== undefined
              ? parseInt(this.skillValues[`effect_${i}_value`])
              : effect.value
        ...
        case 'mentalRecover':
          ...
          if (game.verbose) {
            game.currentTurnLog.push(
              `<div class="log-action log-mental">メンタル回復: +${effect.value} → ${game.mental}%</div>`,
            )
          }
          break
        ...
        case 'mentalReduction':
          ...
          if (game.verbose) {
            game.currentTurnLog.push(
              `<div class="log-action log-mental" style="color: #e74c3c;">${logMessage}: -${reduction} → ${game.mental}%</div>`,
            )
          }
          break
  ...
  evaluateCondition(condition, game) {
    ...
    if (this.cardKey === 'fantasyGin' && condition.includes('mental')) {
      ...
      const resetCount =
        this.skillValues && this.skillValues.mentalThreshold
          ? parseInt(this.skillValues.mentalThreshold)
          : 999
  ...
  processEffects(effects, game, prefix) {
    for (let j = 0; j < effects.length; j++) {
      ...
      switch (effect.type) {
        case 'scoreBoost':
          const scoreBoostValue =
            this.skillValues[key] !== undefined ? parseFloat(this.skillValues[key]) : effect.value
          game.doScoreBoost(scoreBoostValue)
          break
        ...
        case 'voltageGain':
          const voltageGainValue =
            this.skillValues[key] !== undefined ? parseInt(this.skillValues[key]) : effect.value
          game.getVoltage(voltageGainValue)
          break
...
function filterMusicDropdown() {
  ...
  items.forEach((item) => {
    ...
    if (
      ...
      customDesc.includes(searchTerm)
    ) {
      item.style.display = ''
...
function toggleMusicInput() {
  ...
  if (musicSelect.value === 'custom' && previousMusic !== 'custom') {
    ...
    // Set combo counts
    if (previousCombos) {
      if (previousCombos.normal)
        document.getElementById('customComboNormal').value = previousCombos.normal
      if (previousCombos.hard)
        document.getElementById('customComboHard').value = previousCombos.hard
      if (previousCombos.expert)
        document.getElementById('customComboExpert').value = previousCombos.expert
      if (previousCombos.master)
        document.getElementById('customComboMaster').value = previousCombos.master
    }
  }
    ...
    if (musicSelect.value.startsWith('custom_')) {
      ...
      if (savedMusic) {
        ...
        if (savedMusic.combos) {
          if (savedMusic.combos.normal)
            document.getElementById('customComboNormal').value = savedMusic.combos.normal
          if (savedMusic.combos.hard)
            document.getElementById('customComboHard').value = savedMusic.combos.hard
          if (savedMusic.combos.expert)
            document.getElementById('customComboExpert').value = savedMusic.combos.expert
          if (savedMusic.combos.master)
            document.getElementById('customComboMaster').value = savedMusic.combos.master
        }
...
function calculate() {
  ...
  if (musicKey === 'custom' || musicKey.startsWith('custom_')) {
    ...
    musicData[musicKey] &&
    musicData[musicKey].combos &&
    musicData[musicKey].combos[difficulty]
  ) {
    ...
    if (
      customList[musicKey] &&
      customList[musicKey].combos &&
      customList[musicKey].combos[difficulty]
    ) {
  ...
  if (apBalance < 0 && cards.length > 1) {
    ...
    if (apShortageResults.length > 0) {
      for (const result of apShortageResults) {
        const excludedDisplay = result.excludedActivations
          .map((a) => `ターン${a.turn}: ${a.card} (AP${a.apCost})`)
          .join('<br>')
...
function calculateWithAPShortage(appeal, music, totalAP, originalGame, centerCard) {
  ...
  for (
    let i = activationLog.length - 1;
    i >= 0 && excludedCount < excludedActivations.length;
    i--
  ) {
    const activation = activationLog[i]
    if (!activation.isCenterSkill) {
        ...
        const processEffects = (effects) => {
          for (const effect of effects) {
            if (effect.type === 'scoreGain' && effect.value) {
              ...
              const baseScore =
                appeal * actualVoltageMultiplier * boostMultiplier * learningCorrection
    ...
    if (
      ...
      if (cardData[cardValue].centerSkill) {
        ...
        for (let j = 0; j < centerSkill.effects.length; j++) {
          ...
          const effectHtml = generateCenterSkillEffectInputs(
            effect,
            i,
            j,
            '',
        ...
        if (!hasParams) {
          centerSkillHtml +=
            '<div style="color: #666; font-size: 14px;">このセンタースキルには調整可能なパラメータがありません</div>'
        }
      } else {
        centerSkillHtml += `
...
function generateSkillParams(slotNum, cardType, skillLevel = null) {
  ...
  if (!hasParams) {
    skillParams.style.display = 'none'
    html =
      '<div style="color: #666; font-size: 14px;">このカードには調整可能なパラメータがありません</div>'
  } else {
...
function generateCenterSkillEffectInputs(effect, slotNum, effectIndex, prefix, skillLevel = 14) {
  ...
  const inputId = prefix
    ? `centerSkill${slotNum}_${prefix}_${effectIndex}_value`
    : `centerSkill${slotNum}_effect_${effectIndex}_value`
  ...
  switch (effect.type) {
    ...
    case 'skipTurn': // 後方互換性のため残す
      ...
      if (
        cardKey === 'beProudKozu' &&
        effect.condition &&
        effect.condition.includes('skillLevel')
      ) {
    ...
    case 'conditional':
      if (effect.then || effect.else) {
        ...
        if (effect.else && effect.else.length > 0) {
          html += '<div>'
          html +=
            '<div style="font-weight: bold; color: #f44336; margin-top: 5px;">▶ 不成立時</div>'
          for (let i = 0; i < effect.else.length; i++) {
...
function generateEffectInputs(effect, slotNum, effectIndex, prefix, skillLevel = 14) {
  let html = ''
  const inputId = prefix
    ? `skill${slotNum}_${prefix}_${effectIndex}_value`
    : `skill${slotNum}_effect_${effectIndex}_value`
      ...
      if (effect.then || effect.else) {
        ...
        if (effect.then) {
          ...
          for (let j = 0; j < effect.then.length; j++) {
            html += generateEffectInputs(
              effect.then[j],
              slotNum,
              j,
              `effect_${effectIndex}_then`,
              skillLevel,
            )
          }
          html += '</div>'
...
function getCenterSkillValues(slotNum) {
  ...
  const inputs = document.querySelectorAll(
    `.card-slot[data-slot="${slotNum}"] .center-skill-info input[type="number"]`,
  )
...
function updateSaveButtonText() {
  ...
  if (
    currentMusicKey.startsWith('custom_') &&
    customList[currentMusicKey] &&
    customList[currentMusicKey].name === name
  ) {
          ...
          if (version === 'v0') {
            ...
            if (cardIndex >= 0 && cardIndex < cardKeys.length) {
              const cardObj = {
                ...
                centerSkill:
                  centerSkillLevel || parseInt(cardParts[2]) || parseInt(cardParts[1]) || 14,
              }
    ...
    if (data) {
      try {
        ...
        if (data.customMusic && data.customMusicName) {
          ...
          if (data.customCombos) {
            if (data.customCombos.normal)
              document.getElementById('customComboNormal').value = data.customCombos.normal
            if (data.customCombos.hard)
              document.getElementById('customComboHard').value = data.customCombos.hard
            if (data.customCombos.expert)
              document.getElementById('customComboExpert').value = data.customCombos.expert
            if (data.customCombos.master)
              document.getElementById('customComboMaster').value = data.customCombos.master
          }
        }
        ...
        data.cards.forEach((card, index) => {
          if (index < 6) {
            ...
            if (card.id === 'fantasyGin' && card.mentalThreshold !== undefined) {
              setTimeout(() => {
                const mentalThresholdInput = document.getElementById(
                  `skill${slotNum}_mentalThreshold`,
                )
...
function saveAsCustomMusic() {
  ...
  const customName = prompt('カスタム楽曲名を入力してください：', `${musicName} (カスタム編成)`)
  ...
  if (musicValue === 'custom') {
    ...
    difficulties.forEach((diff) => {
      const comboInput = document.getElementById(
        `customCombo${diff.charAt(0).toUpperCase() + diff.slice(1)}`,
      )
      if (comboInput && comboInput.value) {
...
function exitShareMode() {
  ...
  if (originalStateBeforeShare) {
    ...
    document.getElementById('learningCorrection').value =
      originalStateBeforeShare.learningCorrection
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (currentMusic) {
    document
      .querySelector(`.music-dropdown-item[data-value="${currentMusic}"]`)
