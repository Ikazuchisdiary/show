...
const getMusicKey = (music: Music): string => {
  const musicWithId = music as any
...
const compressShareData = (data: any): string => {
  const parts: string[] = []
  ...
  for (const cardInfo of data.cards) {
    ...
    if (cardInfo.centerSkillLevel && cardInfo.centerSkillLevel !== cardInfo.skillLevel && cardInfo.centerSkillLevel !== 14) {
...
const decompressShareData = (compressed: string): any => {
  try {
...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  setMusic: (music) => set((state) => {
    ...
    const musicWithId = state.selectedMusic as any
    ...
    if (state.selectedMusic) {
      ...
      if (musicWithId.id === 'custom' || musicWithId.id?.startsWith('custom_')) {
        ...
        if (state.selectedMusic.combos) {
          data.customCombos = state.selectedMusic.combos
        }
      ...
      if (data) {
        ...
        if (data.music === 'custom') {
          const customMusic: Music = {
            ...
            displayName: data.customMusicName,
            phases: data.customMusic || [11, 7, 5],
            centerCharacter: data.customCenter || '',
            attribute: data.customAttribute || 'smile',
        ...
        data.cards.forEach((cardInfo: any, index: number) => {
          if (index >= 6) return
  ...
  saveCustomMusic: (name, music) => set((state) => {
  ...
  deleteCustomMusic: (key) => set((state) => {
    const newCustomMusicList = { ...state.customMusicList }
